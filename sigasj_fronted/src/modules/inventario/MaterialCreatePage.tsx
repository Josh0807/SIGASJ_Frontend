...
export default function MaterialCreatePage() {
  ...
  const submit = async (values: MaterialFormValues) => {
    try { await createMaterial({ nombre: values.nombre.trim(), descripcion: values.descripcion.trim() || null, unidadMedida: values.unidadMedida.trim(), ubicacion: values.ubicacion.trim() || null, stockMinimo: Number(values.stockMinimo), idCategoria: values.categoriaId ? Number(values.categoriaId) : null, idProveedor: values.proveedorId ? Number(values.proveedorId) : null }) }
