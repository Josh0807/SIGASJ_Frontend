...
describe('contrato API de materiales', () => {
  ...
  it('registra sin enviar stockActual', async () => {
    ...
    vi.stubGlobal('fetch', fetchMock)
    await createMaterial({ nombre: 'Tubo', unidadMedida: 'Unidad', descripcion: null, ubicacion: null, stockMinimo: 0, idCategoria: 2 })
  ...
  it('actualiza datos sin enviar stockActual', async () => {
    const fetchMock = vi.fn().mockResolvedValue(response({ id: 1 }))
    vi.stubGlobal('fetch', fetchMock)
    await updateMaterial(1, { nombre: 'Tubo', unidadMedida: 'Metro', descripcion: null, ubicacion: null, stockMinimo: 2, activo: true, idCategoria: null })
    const options = fetchMock.mock.calls[0][1] as RequestInit
