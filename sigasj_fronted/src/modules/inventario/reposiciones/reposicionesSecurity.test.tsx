...
describe('seguridad de las reposiciones de materiales', () => {
  afterEach(() => {
    vi.unstubAllGlobals()
    localStorage.clear()
  ...
  it('redirige al login sin sesión', async () => {
    const view = await mountAppRoutes('/admin/inventario/reposiciones')
  ...
  it('permite la consulta a la Administradora', async () => {
    ...
    const view = await mountAppRoutes('/admin/inventario/reposiciones')
    expect(view.currentPath()).toBe('/admin/inventario/reposiciones')
    expect(view.container.textContent).toContain('Reposiciones de materiales')
    await view.cleanup()
  })
})
