...
describe('seguridad de las alertas de reposición', () => {
  afterEach(() => {
    vi.unstubAllGlobals()
    localStorage.clear()
  ...
  it('redirige al login sin sesión', async () => {
    const view = await mountAppRoutes('/admin/inventario/alertas-reposicion')
  ...
  it('permite la consulta a la Administradora', async () => {
    ...
    const view = await mountAppRoutes('/admin/inventario/alertas-reposicion')
    expect(view.currentPath()).toBe('/admin/inventario/alertas-reposicion')
    expect(view.container.textContent).toContain('Alertas de reposición')
    await view.cleanup()
  })
})
