...
import { ALERTAS_REPOSICION_PATH, CATEGORIAS_PATH, ENTRADAS_PATH, MATERIAL_NEW_PATH, MOVIMIENTOS_PATH, PROVEEDORES_PATH, RECEPCIONES_PATH, REPORTES_INVENTARIO_PATH, REPOSICIONES_PATH, SALIDAS_PATH, SOLICITUDES_REVISION_PATH, materialEditPath } from './inventarioPaths'
...
export default function MaterialesPage() {
  ...
  const canRegisterExit = canManage || currentRole === InternalAdminRoleName.Fontanero
  ...
  return <main className="materials-admin sigasj-stack">
    <header className="materials-admin__header"><div><p className="materials-admin__eyebrow">Inventario · Bodega</p><h1>Catálogo de materiales</h1><p>Consulte existencias, ubicación y niveles mínimos de los materiales registrados.</p></div><div className="materials-admin__header-actions"><span className="materials-admin__count">{result.total} materiales</span><Link className="materials-admin__secondary" to={CATEGORIAS_PATH}>Categorías</Link><Link className="materials-admin__secondary" to={PROVEEDORES_PATH}>Proveedores</Link>{canRegisterExit && <Link className="materials-admin__primary" to={SALIDAS_PATH}>Registrar salida</Link>}{canManage && <><Link className="materials-admin__secondary" to={SOLICITUDES_REVISION_PATH}>Revisar solicitudes</Link><Link className="materials-admin__secondary" to={ALERTAS_REPOSICION_PATH}>Alertas de reposición</Link><Link className="materials-admin__secondary" to={REPOSICIONES_PATH}>Reposiciones</Link><Link className="materials-admin__secondary" to={RECEPCIONES_PATH}>Recepciones</Link><Link className="materials-admin__secondary" to={MOVIMIENTOS_PATH}>Historial</Link><Link className="materials-admin__secondary" to={REPORTES_INVENTARIO_PATH}>Reportes</Link><Link className="materials-admin__secondary" to={ENTRADAS_PATH}>Registrar entrada</Link><Link className="materials-admin__primary" to={MATERIAL_NEW_PATH}>Nuevo material</Link></>}</div></header>
    <section className="materials-admin__filters materials-admin__filters--inventory" aria-label="Búsqueda y filtros"><label><span>Buscar por nombre</span><input type="search" value={searchInput} maxLength={150} placeholder="Ej. Tubo PVC" onChange={(event) => setSearchInput(event.target.value)} /></label><label><span>Estado</span><select value={activo} onChange={(event) => { setActivo(event.target.value); setPage(1) }}><option value="">Todos</option><option value="true">Activos</option><option value="false">Inactivos</option></select></label><label><span>Categoría</span><select value={categoriaId} disabled={categoriasLoading} onChange={(event) => { setCategoriaId(event.target.value); setPage(1) }}><option value="">Todas las categorías</option>{categorias.data.map((item) => <option key={item.id} value={item.id}>{item.nombre}{item.activo ? '' : ' (inactiva)'}</option>)}</select></label><label><span>Proveedor</span><select value={proveedorId} disabled={proveedoresLoading} onChange={(event) => { setProveedorId(event.target.value); setPage(1) }}><option value="">Todos los proveedores</option>{proveedores.data.map((item) => <option key={item.id} value={item.id}>{item.nombre}{item.activo ? '' : ' (inactivo)'}</option>)}</select></label>{filtered && <button type="button" className="materials-admin__secondary" onClick={clearFilters}>Limpiar filtros</button>}{categoriasError && <div className="materials-admin__category-error" role="alert">No fue posible cargar el filtro de categorías. <button type="button" onClick={refetchCategorias}>Reintentar</button></div>}{proveedoresError && <div className="materials-admin__category-error" role="alert">No fue posible cargar el filtro de proveedores. <button type="button" onClick={refetchProveedores}>Reintentar</button></div>}</section>
