...
export default function MaterialEditPage() {
  ...
  const initialValues: MaterialFormValues = { nombre: material.nombre, descripcion: material.descripcion ?? '', unidadMedida: material.unidadMedida, ubicacion: material.ubicacion ?? '', stockMinimo: String(material.stockMinimo), activo: material.activo, categoriaId: material.idCategoria ? String(material.idCategoria) : '' }
  const submit = async (values: MaterialFormValues) => {
    try { await updateMaterial(validId, { nombre: values.nombre.trim(), descripcion: values.descripcion.trim() || null, unidadMedida: values.unidadMedida.trim(), ubicacion: values.ubicacion.trim() || null, stockMinimo: Number(values.stockMinimo), activo: values.activo, idCategoria: values.categoriaId ? Number(values.categoriaId) : null }) }
