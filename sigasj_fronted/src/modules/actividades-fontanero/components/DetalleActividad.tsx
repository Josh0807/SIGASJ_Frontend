...
const DetalleActividad = ({
  ...
  if (loading) {
    return (
      <div className="detalle-actividad__state" role="status" aria-busy="true">
        Cargando detalle…
      </div>
    )
  }
  ...
  if (error || !actividad) {
    return (
      <div className="detalle-actividad__state detalle-actividad__state--error">
        <ActivityFeedback
          variant="warning"
          message={error || ACTIVITY_FEEDBACK_MESSAGES.notFound}
        />
        {onBack ? (
  ...
  return (
    <article
      className="detalle-actividad actividades-fontanero-historial-detalle__panel"
      aria-labelledby="detalle-actividad-title"
    >
      <header className="detalle-actividad__header">
        ...
        <div className="detalle-actividad__badges">
          <span data-estado={actividad.estado}>
            {formatActividadEstado(actividad.estado)}
          </span>
          <span className={revisada ? 'is-reviewed' : 'is-pending'}>
            {revisionLabel(actividad)}
          </span>
        </div>
      </header>
      ...
      <section className="detalle-actividad__section">
        ...
        <dl className="detalle-actividad__grid">
          <div>
            <dt>Tipo</dt>
            <dd>{actividad.tipoActividadNombre || '—'}</dd>
          </div>
          <div>
            <dt>Fecha</dt>
          ...
          <div>
            ...
            <dd>{formatActividadEstado(actividad.estado)}</dd>
          </div>
          <div>
            <dt>Estado de revisión</dt>
            <dd>{revisionLabel(actividad)}</dd>
          </div>
          {actividad.fechaRegistro || actividad.createdAt ? (
            <div>
          ...
          {revisada && actividad.fechaRevision ? (
            <div>
              <dt>Fecha de revisión</dt>
              <dd>{new Date(actividad.fechaRevision).toLocaleString('es-CR')}</dd>
            </div>
          ) : null}
          {revisada && actividad.revisadoPorId ? (
            <div>
              <dt>Revisada por</dt>
              <dd>{actividad.revisadoPorId}</dd>
            </div>
          ) : null}
        </dl>
      </section>

      {actividad.descripcion || actividad.ubicacion ? (
        <section className="detalle-actividad__section">
      ...
      <section className="detalle-actividad__section">
        <h3>Observaciones</h3>
        <p>{actividad.observaciones || 'Sin observaciones.'}</p>
      </section>

      {actividad.observacionCorreccion ? (
        <section
          className="detalle-actividad__section detalle-actividad__correction"
          role="note"
        >
          <h3>Corrección solicitada</h3>
      ...
      <section className="detalle-actividad__section">
        ...
        {actividad.documentos?.length ? (
          <ul className="detalle-actividad__documents">
            {actividad.documentos.map((documento) => (
              <li key={documento.id}>
                <div>
                  <strong>{documento.nombreOriginal}</strong>
                  <small>
                    {Math.max(1, Math.ceil(documento.tamanio / 1024))} KB
                  </small>
                </div>
                <a
                  href={documentoUrl(documento.rutaReferenciaArchivo)}
                  target="_blank"
                  rel="noreferrer"
                >
                  Ver documento
      ...
      {onBack || actions ? (
        <footer className="detalle-actividad__actions">
          {onBack ? (
            <button type="button" onClick={onBack}>
              {backLabel}
            </button>
