...
import { getFontaneroAveria } from '../services/averiasFontaneroApi'

export type UseFontaneroAveriaResult = {
...
export function useFontaneroAveria(
  id: string | number | null | undefined,
  options: { enabled?: boolean } = {},
): UseFontaneroAveriaResult {
  const enabled = options.enabled ?? true
  const parsedId = parseAveriaAdminId(id)
  const canFetch = enabled && parsedId !== null
  const [averia, setAveria] = useState<AveriaFontaneroDetail | null>(null)
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        const result = await getFontaneroAveria(parsedId, controller.signal)
        if (!cancelled) {
          setAveria(result)
        }
      } catch (caught) {
        ...
        if (parsed.kind === 'unauthorized') {
          ...
          setError(AVERIAS_FONTANERO_DETAIL_ERROR)
        }
      } finally {
        if (!cancelled) {
