...
} from '../../../shared/api/transparencia/adminTransparencia'
import { getAccessToken, clearAccessToken } from '../../auth/services/authStorage'
import TransparenciaAdminForm from '../components/TransparenciaAdminForm'
import { useAdminTransparencia } from '../hooks/useAdminTransparencia'
import {
...
const TransparenciaAdminPage = () => {
  ...
  const publicationsQuery = useAdminTransparencia(filters)
  const publications = publicationsQuery.data ?? []
  const status = publicationsQuery.isError
    ? 'error'
  ...
  const refreshPublications = async () => {
    await queryClient.invalidateQueries({ queryKey: ['transparencia'] })
  }
      ...
      <div className="gallery-admin__shell">
        ...
        {formMode !== 'hidden' ? (
          <TransparenciaAdminForm
            key={`${formMode}-${editingPublication?.id ?? 'new'}`}
            mode={formMode}
        ...
        {status === 'loading' ? (
          ...
          <div className="gallery-admin__empty" role="alert">
            ...
            <button type="button" onClick={() => void publicationsQuery.refetch()}>
              Reintentar
            </button>
          </div>
