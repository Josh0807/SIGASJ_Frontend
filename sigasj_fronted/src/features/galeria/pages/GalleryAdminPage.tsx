...
import GalleryAdminForm from '../components/GalleryAdminForm'
import { useAdminGalleryPhotos } from '../hooks/useAdminGalleryPhotos'
import {
...
const GalleryAdminPage = () => {
  ...
  const photosQuery = useAdminGalleryPhotos(filters)
  const photos = photosQuery.data ?? []
  const status = photosQuery.isError
    ? 'error'
  ...
  const handleLogout = () => {
    clearAccessToken()
    void queryClient.invalidateQueries({ queryKey: queryKeys.galeria.admin(filters) })
  ...
  return (
    <main className="gallery-admin">
      <div className="gallery-admin__shell">
        ...
        {formMode !== 'hidden' ? (
          <GalleryAdminForm
            key={`${formMode}-${editingPhoto?.id ?? 'new'}`}
            mode={formMode}
